"""
engines/image_engine.py — PDFWala Enterprise V14.0
...
try:
    ...
    # FIX V14: 500 MP guard (raised from 200MP) to handle 5000-page high-DPI scans
    Image.MAX_IMAGE_PIXELS = getattr(Config, "MAX_IMAGE_PIXELS", 500_000_000)
    PIL_OK = True
except ImportError:
    PIL_OK = False
...
import re as _re
_VALID_TESSERACT_LANGS = _re.compile(r'^[a-zA-Z]{2,8}(\+[a-zA-Z]{2,8})*$')


def _sanitise_tesseract_lang(lang: str) -> str:
    """Whitelist-validate a Tesseract language code. Prevents flag injection."""
    lang = lang.strip()
