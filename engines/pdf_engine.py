"""
engines/pdf_engine.py — PDFWala Enterprise V13.0
...
def organize_pdf(ctx: JobContext) -> dict:
    ...
    try:
        ...
        out_doc = fitz.open()
        out_doc.insert_pdf(src_doc, from_page=-1, to_page=-1)  # start empty
        # remove that placeholder; build from keep list
        out_doc2 = fitz.open()
        for idx in keep:
...
def compress_pdf(ctx: JobContext) -> dict:
    ...
    try:
        ...
        modified = compress_pdf_images(
            ...
            max_image_bytes=_IMAGE_RAM_GUARD_BYTES,
        )
        if modified:
...
def linearize_pdf(ctx: JobContext) -> dict:
    ...
    if QPDF_OK:
        timeout = getattr(Config, "SUBPROCESS_TIMEOUT", 120)
        try:
...
def pdf_to_word(ctx: JobContext) -> dict:
    ...
    if not os.path.exists(ctx.output_path) or os.path.getsize(ctx.output_path) == 0:
        raise ProcessingError("pdf2docx produced empty output — conversion may have failed")
    return {}
...
def pdf_to_ppt(ctx: JobContext) -> dict:
    ...
    try:
        ...
        for i, page in enumerate(doc):
            ...
            if i % 10 == 0:
                ctx.set_progress(int(i / len(doc) * 90))
    finally:
        doc.close()
    ...
    log.info(f"[{ctx.job_id}] pdf_to_ppt: {len(doc)} slides created")
    return {}
...
def pdf_to_pdfa(ctx: JobContext) -> dict:
    ...
    timeout = getattr(Config, "PDFA_TIMEOUT", Config.SUBPROCESS_TIMEOUT)
    try:
        result = subprocess.run(cmd, capture_output=True, timeout=timeout)
        if result.returncode != 0:
...
def ocr_pdf(ctx: JobContext) -> dict:
    ...
    workers = getattr(Config, "OCR_WORKERS", min(4, (os.cpu_count() or 2)))
