"""
engines/office_engine.py — PDFWala Enterprise V14.0
...
_EXCEL_ROW_LIMIT      = Config.EXCEL_ROW_LIMIT
_EXCEL_COL_LIMIT      = Config.EXCEL_COL_LIMIT
_MAX_WORD_PARAGRAPHS  = Config.MAX_WORD_PARAGRAPHS
_MAX_WORD_TABLES      = Config.MAX_WORD_TABLES
_MAX_PPT_SLIDES       = Config.MAX_PPT_SLIDES
_MAX_PPT_SLIDE_ROWS   = Config.MAX_PPT_SLIDE_ROWS
...
def _libre(
    ...
    output_root  = str(Path(getattr(Config, "OUTPUT_FOLDER", tmp_root)).resolve())
    ...
    try:
        # FIX V14: Set headless env vars to prevent LibreOffice from trying to
        # connect to a display server (hangs in Docker without this)
        lo_env = os.environ.copy()
        lo_env["SAL_USE_VCLPLUGIN"] = "svp"    # software renderer — no X11 needed
