"""
tasks/office_tasks.py — PDFWala Enterprise V14.0
...
@celery_app.task(base=_BaseTask, name="tasks.excel_to_html",  bind=True)
def task_excel_to_html(self, job_id):  _run_job(job_id)

@celery_app.task(base=_BaseTask, name="tasks.excel_to_png",   bind=True)
def task_excel_to_png(self, job_id):   _run_job(job_id)

@celery_app.task(base=_BaseTask, name="tasks.html_to_pdf",    bind=True)
def task_html_to_pdf(self, job_id):    _run_job(job_id)
...
OFFICE_TASK_MAP = {
    ...
    "excel_to_html":  task_excel_to_html,
    "excel_to_png":   task_excel_to_png,
    "html_to_pdf":    task_html_to_pdf,
}
