...
Fixes vs V10:
  - -dNOSAFER replaced with -dSAFER (security fix CRIT-05, matches app.py)
  - time_limit / soft_time_limit added to all tasks
  - retry countdown capped (was 30 * 3^retries = 810s on 3rd retry — too long)
  - None-guard on celery_app removed: tasks are always registered because
    celery_app is always created in workers/celery_app.py
"""
...
def _ghostscript_compress(
    ...
    if not cb_ghostscript.can_execute():
        ...
        return False
    # FIX CRIT-05: -dSAFER (not -dNOSAFER) — prevents arbitrary file access via PS
    cmd = [
        Config.GHOSTSCRIPT,
        "-sDEVICE=pdfwrite",
...
def compress_pdf_task(
    ...
    try:
        ...
        # FIX: cap retry delay — original formula gave 810s on 3rd retry
