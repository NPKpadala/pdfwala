"""
tasks/pdf_tasks.py — PDFWala Enterprise V14.0

V14 FIXES over V13:
  - SoftTimeLimitExceeded now caught in ALL task runners (was only imported in pdf_tasks,
    never handled — caused tasks to die without marking job as failed in Redis)
  - _run_job marks job as "timeout" status on SoftTimeLimitExceeded
  - Metrics recorded even on SoftTimeLimitExceeded
"""
...
def _run_job(job_id: str) -> None:
    ...
    try:
        ...
        # FIX V14: Catch soft timeout — mark job failed with clear message
        ok = False
